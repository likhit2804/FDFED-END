...
if (missingVars.length > 0) {
  console.warn(`âš ï¸ Missing email configuration: ${missingVars.join(', ')}`);
}
...
async function sendEmail({ to, subject, html, text = '' }) {
  try {
    ...
    console.log(`âœ… Email sent to ${to}: ${subject} (ID: ${info.messageId})`);
    return true;
  } catch (error) {
 ...
 * @param {string} options.icon - Header icon (default: ðŸ””)
...
export async function sendNotificationEmail(email, title, message, options = {}) {
  const { icon = 'ðŸ””', type = 'info' } = options;
...
export function closeEmailService() {
  if (transporter) {
    transporter.close();
    transporter = null;
    console.log('ðŸ“§ Email service closed');
  }
}
