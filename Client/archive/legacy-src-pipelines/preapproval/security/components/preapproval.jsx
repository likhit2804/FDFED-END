...
import "../../../assets/css/GlobalStyles/preapproval.css";
...
export function SecurityPreApproval() {
  ...
  const onScanSuccess = async (decodedText) => {
    try {
      const data = await securityService.verifyQR(decodedText);
      ...
      alert(
        `Visitor ${data.visitor.name} successfully ${data.visitor.status === "Active" ? "Checked In" : "Checked Out"
        }`
      );
  ...
  const fetchData = async () => {
    try {
      const data = await securityService.getPreApprovals();
  ...
  const handleAction = async (id, status) => {
    try {
      const data = await securityService.updatePreApprovalStatus(id, status);
      if (data.success) {
        fetchData();
      }
    } catch (err) {
