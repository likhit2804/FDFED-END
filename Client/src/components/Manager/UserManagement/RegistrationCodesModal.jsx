...
export const RegistrationCodesView = ({
  communityName,
  codesList = [],
  codesLoading = false,
  codesSearch = "",
    ...
    <div className="rcm">
      <div className="rcm__header">
        ...
        <div className="rcm__stats">
          ...
          <span className="manager-ui-status-pill">{selectedFlats?.size || 0} selected</span>
        </div>
      </div>
      <div className="manager-ui-toolbar rcm__toolbar">
        ...
        <button
          ...
          disabled={!selectedFlats || selectedFlats.size === 0 || isRegenerating}
          onClick={() => regenerateCode(null, Array.from(selectedFlats))}
        >
          <RefreshCw size={15} className={isRegenerating ? "rcm__spin" : ""} />
          {isRegenerating ? "Regenerating..." : `Regenerate (${selectedFlats?.size || 0})`}
        </button>
      </div>
      {codesLoading ? (
        <div className="manager-ui-empty"><Loader label="Fetching registration codes..." /></div>
      ) : codesList.length === 0 ? (
        <div className="manager-ui-empty rcm__empty" style={{ padding: "48px 24px", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <Home size={40} style={{ color: "var(--brand-500)", marginBottom: 14 }} />
          <h4 style={{ margin: "0 0 8px", fontSize: 17, fontWeight: 600 }}>No Flats or Resident Codes Yet</h4>
          <p style={{ margin: "0 0 20px", fontSize: 13.5, color: "var(--text-subtle)", maxWidth: 460 }}>
            Run Community Setup to configure blocks, floors, and flats. UrbanEase will automatically generate unique resident registration codes for every unit.
          </p>
          {onNavigateSetup ? (
            <button type="button" className="manager-ui-button manager-ui-button--primary" onClick={onNavigateSetup}>
              Go to Community Setup
            </button>
          ) : null}
        </div>
      ) : (
        <div className="rcm__table-wrap">
          <table className="rcm__table">
            ...
            <tbody>
              {filtered.map((flat) => {
                const isSelected = Boolean(selectedFlats?.has(flat.flatNumber));
                return (
          ...
          {filtered.length === 0 ? (
            <div className="manager-ui-empty rcm__empty" style={{ padding: "30px", textAlign: "center" }}>
